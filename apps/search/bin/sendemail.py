...
def getEmailList(email):
    tmpList = sec.EMAIL_DELIM.split(email)
    emailList = []
    for email_item in tmpList:
        if re.search(sec.EMAIL_FORMAT, email_item):
            emailList.append(email_item)
        else:
            logger.warning("Invalid email recipient=%s, remove recipient=%s from recipients."
...
def sec_mail(sessionKey, email, emailSettings, argvals, ssContent, sid):
    '''
    Construct the email message and attempt to send the email message for the splunk stack. Applicable to Splunk Cloud only.
    ...
    @param emailSettings: email settings from config
    @type emailSettings: dict

    @param argvals: args including username and password for any mailserver login
    @type argvals: dict

    @param ssContent: Saved search content that contains information about the saved search derived from savedsearches.conf
    @type ssContent: dict

    @param sid: splunk search ID
    @type sid: str
    ...
    logCtx = 'savedSearchName="%s", savedSearchOwner="%s", savedSearchApp="%s",' \
             ' sid="%s", subject="%s", encoded_subject="%s", results_link="%s"' % (
             ssContent.get('name', ''), ssContent.get('owner', ''), ssContent.get('app', ''),
             sid, ssContent.get('action.email.subject', ''), email.get('subject', ''),
             ssContent.get('results_link', ''))
    sec.SplunkEmailMessage(sessionKey,
                           email,
                           ssContent.get('action.email.allowedDomainList', ""),
                           logCtx,
                           emailSettings,
                           cLogger=logger).send(
                                use_tls,
                                use_ssl,
                                ssContent.get('alertActions', {}),
                                username,
                                password,
                                server)
    return

def mail(sessionKey, email, argvals, ssContent, sid):
    '''
    Construct the email message and attempt to send the email message
    ...
    if isSCEmailValidationEnabled:
        ...
        if validation in ('strict', 'enhanced'):
            sec_mail(sessionKey, email, emailSettings, argvals, ssContent, sid)
            return
    # basic validation uses existing dispatch route
    ...
    if ssContent.get('action.email.allowedDomainList') != "" and ssContent.get('action.email.allowedDomainList') != None:
        ...
        # SPL-259247 - special handling for allowedDomainList = allow_all 
        #              allowedDomainList = deny_all 
        ...
        domainsRestriction = sec.checkPreDefinedDomains(domains, logger, sender, recipients,logCtx)
        if domainsRestriction == sec.ALLOWED_DOMAIN_LIST_ALLOW_ALL:
            validRecipients = recipients
        elif domainsRestriction == sec.ALLOWED_DOMAIN_LIST_DENY_ALL:
            raise Exception(sec.ALLOWED_DOMAIN_LIST_DENY_ALL_MSG)
        else:           
            for recipient in recipients:
                dom = recipient.partition("@")[2]
                if not dom in domains:
                    logger.error("For subject=%s, email recipient=%s is not among the allowedDomainList=%s in alert_actions.conf file. Removing it from the recipients list."
    ...
    mail_log_msg = 'Sending email. sid=%s, subject="%s", encoded_subject="%s", results_link="%s", recipients="%s", server="%s"' % (
        sid,
    ...
    try:
        ...
        smtpConfig = SMTPConfig(logCtx, server, sessionKey, use_ssl, use_tls, ssContent.get('alertActions', {}))
        ...
        if SMTPOAuth.isOAuthEnabled(sessionKey, logger):
            ...
            smtp.authenticate()
        else: # SMTP servers with no authentication
        ...
        if isSCLimitsEnabled:
            sec.SplunkEmailMessage.enforceLimits(validRecipients, emailSettings, logger, mail_log_msg)

        # Installed SMTP daemon may not support UTF8 as well as
        # it may throw other exceptions also.
        error = sec.sendEmailWithUTF8(smtp.smtp_connection, sender, validRecipients, email.as_string())

        if (error is not None):
            logger.debug('send mail with utf8 failed. retrying without utf8 option. Error: %s', str(error))
            smtp.smtp_connection.sendmail(sender, validRecipients, email.as_string())
...
def sendEmail(results, settings, keywords, argvals):
    ...
    if ssname:
        ...
        if normalizeBoolean(ssContent['results_link']) and normalizeBoolean(ssContent['type']):
            uri_tuple = urllib.urlparse(ssContent.get('results_link'))
            path_minus_last_segment = '/'.join(uri_tuple.path.split('/')[:-1])
            new_uri_parts = [uri_tuple.scheme, uri_tuple.netloc, path_minus_last_segment, None, None, None]
            view_path = urllib.urlunparse(new_uri_parts) + "/"
            ssType = ssContent.get('type')
            useGoLink = False
            if '@go' in uri_tuple.path:
                useGoLink = True
            if ssType == 'alert':
                if is_stream_malert:
                    # SPL-174186
    ...
    if argvals.get('newLineValuesInCSV'):
        ssContent['action.email.newLineValuesInCSV'] = normalizeBoolean(argvals.get('newLineValuesInCSV'))
    ...
    if argvals.get('server'):
        ...
        if alertContent.get('allowedDomainList') == "":
            ...
            logger.warning("When 'allowedDomainList' is setup, 'server' argument is not accepted in sendemail command. "
    ...
    if argvals.get('results_file'):
        ...
        if dirname.endswith('per_result_alert'):
            ...
            if results_ids and len(results_ids) > 0:
                ...
                logger.warning('Cannot parse results_id=%s for per result alert' % (fname))
    ...
    if ssContent.get('action.email.allowedDomainList'):
        ...
        if ssContent.get('action.email.allowedDomainList') != alertContent.get('allowedDomainList'):
            ...
            logger.warning("For alert=%s, the 'allowedDomainList' value is always obtained from alert_actions.conf."
    ...
    if alertContent.get('allowedDomainList') != "":
        if ssContent.get('action.email.mailserver') != alertContent.get('mailserver'):
            ...
            logger.warning("For alert=%s, if a 'allowedDomainList' is specified, it uses the 'mailserver'=%s in alert_actions.conf." %
    ...
    sec.renderTime(resultsWithRenderedTime)
    ...
    try:
        mail(sessionKey, email, argvals, ssContent, sid)
    except Exception as e:
...
def buildAttachments(settings, ssContent, results, email, jobCount, argvals):
    ...
    if len(results) == 0 and not allowEmpty:
        ...
        newLineValuesInCSV = normalizeBoolean(ssContent.get('action.email.newLineValuesInCSV', False))
        csvAttachment.set_payload(generateCSVResults(results, escapeCSVNewline, newLineValuesInCSV))
...
def generateCSVResults(results, escapeCSVNewline, newLineValuesInCSV):
    if len(results) == 0:
   ...
   try:
      ...
      if 'auth_username' in ent and 'clear_password' in ent:
          ...
          if splunkhome == None:
              ...
              return None
          clear_password = cli_common.decrypt(encrypted_password, setEnv=True, ignoreErrors=True)
...
def sendHealthAlertEmail(results, settings):
    ...
    # 'settings' represents alert_actions settings
    ...
    try:
        mail(sessionKey, email, argvals, alertConfig, None)
    except Exception as e:
...
if __name__ == '__main__':
    ...
    size_of_results = sec.get_approx_nested_sizeof(results)
    logger.info('SENDEMAIL full size of results %d' % (size_of_results))
    try:
        ...
        if 'is_health_alert' in argvals:
            ...
            if results or 'ssname' in argvals:
                ...
                logger.warning("search results is empty, no email will be sent")
    except Exception as e:
        logger.exception(e)
